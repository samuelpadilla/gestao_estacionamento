...
Antes de criar uma nova _migration_, é necessário realizar os seguintes passos:
 ...
 - [x] Adicionar o contexto da entidade em **GestDbContext.cs** (GEST.Infraestructure/Persistence)
...
```bash
Add-Migration NomeMigracao -Context GestDbContext
```

### Atualizar o banco de dados

```bash
Update-Database -Context GestDbContext
```

### Reverter a última migração

```bash
Remove-Migration -Context GestDbContext
```